...
% $Revision: 1.1.10.10 $  $Date: 2011/05/17 02:20:59 $
...
elseif isfield(S,'Lon') && isfield(S,'Lat')
    ...
    error(message('map:shapefile:missingCoordinateField'))
end
...
if fid < 0
    error(message('map:shapefile:failedToOpenFile', [ basename, '.shx' ]))
end
...
if fid < 0
    error(message('map:shapefile:failedToOpenFile', [ basename, '.dbf' ]))
end
...
numRequiredArgs = 2;
error(nargchk(numRequiredArgs,...
    numRequiredArgs + 2*numel(validParameterNames), nargin, 'struct'));
...
S = varargin{1};
if isempty(S)
    error(message('map:shapefile:emptyGeostruct'))
end
...
for k = 3:2:nargin
    ...
    switch parName
        ...
        otherwise
            error(message('map:shapefile:internalProblem', parName));
    end
end
...
if ~isstruct(dbfspec)
    error(message('map:shapefile:dbfspecNotAStructure'))
end
...
if ~isempty(missingAttributes)
    error(message('map:shapefile:missingAttributes'))
end
...
for k = 1:numel(attributeNamesInDbfSpec)
    ...
    elseif fSpec.FieldType ~= fDefault.FieldType
        error(message('map:shapefile:extaneousAttributes', attributeName))
    end
...
if ~isfield(S, 'Geometry')
    error(message('map:shapefile:noGeometry'));
end
...
if ~any(strcmp(S1Geometry,validGeometries))
    error(message('map:shapefile:invalidGeometryString'))
end
...
for k = 1:numel(S)
    if ~strcmp(S(k).Geometry,S(1).Geometry)
        error(message('map:shapefile:inconsistentGeometry'));
    end
    ...
    if ~isa(c1,'double') || ~isa(c2,'double')
        error(message('map:shapefile:nonDoubleCoordinate'))
    end
    
    % Make sure they match in length
    if ~isequal(size(c1),size(c2))
        error(message('map:shapefile:coordinateSizeMismatch'))
    end
    
    % Make sure they have matching NaNs, in any
    if isnan(c1) ~= isnan(c2)
        error(message('map:shapefile:coordinateNanMismatch'))
    end
...
if strcmp(S(1).Geometry,'Point')
    for k = 1:numel(S)
        assert(isscalar(S(k).(cfield1)) && isscalar(S(k).(cfield2)), ...
            'map:shapefile:nonScalarPoint', ...
            'Coordinates in ''Point'' geographic data structure S must be scalar.')
    end
end
