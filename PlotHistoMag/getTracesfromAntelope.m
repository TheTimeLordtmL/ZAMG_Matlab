function [dataV,dataA,setting,error] = getTracesfromAntelope(setting,pcurr,psum)
% pcurr..index current    psum...index max
dataV = [];   dataA = [];  data = [];

%// get db querry strings
strstation = sprintf('%s',setting.station); 
fprintf('..start getting traces from Antelope DB for station %s at %s \n',strstation,setting.time.start);
...
for z=1:numel(setting.comp)
  ...
  if n > 0
     ...
     trdestroy(tr);
     fprintf('..(%g/%g) %s  %s found with %4.0f samprate and %g nsamp \n',pcurr,psum,strstation,curr_component,samprate,nsamp);
     %subplot(3,1,z);
     %plot(data);
     %legend();
  else
